...
function MagicFunction(...args) {
  ...
  function f(...innerArgs) {
    result += sum(innerArgs);

    return f;
  }

  // to convert to a number inside an expression
  f.valueOf = () => result;
  debugger;
  return f;
}
...
function sum(values) {
  ...
  for (const value of values) {
    // we take only values that are not NaN or 0
    result += +value || 0;
  }
...
console.log(MagicFunction(2, 3)(3, 4));




// const a = MagicFunction;
// console.log(a);
